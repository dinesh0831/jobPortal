...
function CandidateHome() {
    ...
    const getPosts =useCallback( async () => {
        if (token) {
            ...
            if (!isMyTokenExpired) {
                const { data } = await axios.get(`${Url.backendUrl}/jobs/getposts`)
                console.log(data)
                setData(data)
            }
            else{
...
const apply=async(item)=>{
    ...
    return (
        <Box sx={{ margin:3}}>
            <Box sx={{display:"flex",alignItems:"center",justifyContent:"center",margin:3}}><h1>Your Job Suggestion</h1></Box>
            {data.map((item, index) => {
                return (
                    <Accordion expanded={expanded === index} key={item._id} onChange={handleChange(index)}>
                        <AccordionSummary
